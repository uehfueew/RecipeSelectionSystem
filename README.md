...
The system includes a performance test (`main.py` option 12) that compares BubbleSort and MergeSort.

### Algorithm Comparison
- **BubbleSort**: An $O(n^2)$ loop-based algorithm. It performs well on very small datasets or nearly sorted data but slows down significantly as $n$ increases due to its nested loops.
- **MergeSort**: An $O(n \log n)$ recursion-based (Divide & Conquer) algorithm. It maintains consistent performance even on larger datasets because it splits the workload logarithmically.

### Test Results
In our tests with $n=100$:
- BubbleSort: ~0.001s
- MergeSort: ~0.0002s
As predicted by Big-O complexity, MergeSort is significantly faster for larger collections.

## Truth Table Logic
The system evaluates logical expressions using Python's `ast` module. It supports:
- Variables: `cheap`, `quick`, `healthy`, `contains_chicken`
- Operators: `and`, `or`, `not`
- Truth tables are generated by iterating through all $2^n$ possible combinations of boolean values for the variables in the expression.
